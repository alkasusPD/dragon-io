...
function updateHud(){if(!player)return;hud.xp.style.width=`${clamp(player.xp/player.need,0,1)*100}%`;hud.level.textContent=t('hud.level',{level:player.level});hud.score.textContent=t('hud.score',{score:score.toString().padStart(5,'0')});updateWaveProgress();}
...
function drawEnemyHealthBar(e){
  ...
  ctx.save();ctx.translate(e.x,e.y);ctx.fillStyle='#18252bdd';ctx.strokeStyle=e.boss?'#ffcf4e':e.elite?'#9aa9b0':'#263a44';ctx.lineWidth=e.boss?2.5:e.elite?2:1.5;ctx.beginPath();ctx.roundRect(-width/2-2,y-2,width+4,height+4,height);ctx.fill();ctx.stroke();ctx.fillStyle='#33484f';ctx.fillRect(-width/2,y,width,height);if(ratio>0){ctx.fillStyle=ratio<.3?'#ff5261':e.boss?'#ff7b31':e.elite?'#ffcf4e':'#8fe94b';ctx.fillRect(-width/2,y,width*ratio,height);ctx.fillStyle='#ffffff';ctx.globalAlpha=.5;ctx.fillRect(-width/2+2,y+1,Math.max(0,width*ratio-4),1);}ctx.restore();
}
...
function draw(){
  ...
  drawVirtualJoystick();
  if(waveBanner>0){const fade=Math.min(1,waveBanner/.45,(2.4-waveBanner)/.35);ctx.save();ctx.globalAlpha=Math.max(0,fade);ctx.textAlign='center';ctx.fillStyle='#09293bc9';ctx.roundRect(W/2-112,155,224,72,20);ctx.fill();ctx.strokeStyle='#c9ff7c';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle='#fff';ctx.shadowBlur=14;ctx.shadowColor='#69eaff';ctx.font='900 34px system-ui';ctx.fillText(t('hud.wave',{wave}),W/2,201);ctx.restore();}
}
