...
art.chapterBoss.src = 'assets/sprites/emerald-dreadwing-phase2-v3.png';
...
function drawChapterBossAura(x,y,size,phase){const pulse=.96+Math.sin(animationTime*2.4+phase)*.04;ctx.save();ctx.translate(x,y);ctx.globalCompositeOperation='lighter';ctx.scale(1,.72);const glow=ctx.createRadialGradient(0,0,size*.08,0,0,size*.43);glow.addColorStop(0,'#d8a0ff55');glow.addColorStop(.48,'#8d32e833');glow.addColorStop(1,'#4d0b9b00');ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,0,size*.43*pulse,0,Math.PI*2);ctx.fill();ctx.scale(1,1/.72);for(let ring=0;ring<2;ring++){ctx.save();ctx.rotate((ring?1:-1)*animationTime*(.48+ring*.16)+phase);ctx.strokeStyle=ring?'#d986ff88':'#7b35e899';ctx.lineWidth=ring?2.2:3;ctx.setLineDash(ring?[11,20]:[22,28]);ctx.beginPath();ctx.arc(0,0,size*(.29+ring*.065)*pulse,0,Math.PI*2);ctx.stroke();ctx.restore();}ctx.fillStyle='#e8b6ff';for(let i=0;i<6;i++){const a=animationTime*(.7+i%2*.18)+i*Math.PI/3+phase,r=size*(.28+(i%3)*.035),spark=2+Math.sin(animationTime*4+i)*.8;ctx.globalAlpha=.55+.3*Math.sin(animationTime*3+i);ctx.beginPath();ctx.arc(Math.cos(a)*r,Math.sin(a)*r*.72,spark,0,Math.PI*2);ctx.fill();}ctx.restore();}
function drawMonsterVisual(e,frame,y,size,rotation){if(e.chapterBoss){drawChapterBossAura(e.x,y,size,e.phase);const wingBeat=Math.sin(animationTime*3.4+e.phase),breathe=1+wingBeat*.022;drawSprite(art.chapterBoss,frame,e.x,y,size,rotation+wingBeat*.012,1,e.hitFlash||0,breathe,2-breathe);return;}if(e.boss){drawSprite(art.miniBoss,frame,e.x,y,size,rotation,1,e.hitFlash||0);return;}const visual=monsterVisuals[e.monsterId]||monsterVisuals.wyvern;drawSprite(art[visual.baseArt],frame,e.x,y,size,rotation,1,e.hitFlash||0);if(e.elite&&visual.elitePartsArt)drawSprite(art[visual.elitePartsArt],frame,e.x,y,size,rotation,1,e.hitFlash||0);}
...
function draw(){
  ...
  enemies.forEach(e=>{const frame=Math.floor(e.anim)%12,size=e.chapterBoss?310:e.boss?240:e.champion?180:e.elite?150:108,flap=Math.sin(e.anim*Math.PI/6),bob=flap*(e.chapterBoss?4.8:1.2),aiming=lasers.some(l=>l.source===e),flightTilt=aiming?0:Math.sin(time*(e.chapterBoss?1.35:2)+e.phase)*(e.chapterBoss?.026:.018),rotation=e.rotation+flightTilt;drawMonsterVisual(e,frame,e.y+bob,size,rotation);drawPoisonCoating(e);drawEnemyHealthBar(e);});
  ...
  enemyShots.forEach(s=>{if(s.type!=='chapterHoming'||s.charge<=0)return;const chargeProgress=1-s.charge/.9,pulse=1+Math.sin(animationTime*16)*.08;ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle='#c967ff';ctx.shadowBlur=18;ctx.shadowColor='#a72cff';ctx.globalAlpha=.35+chargeProgress*.55;ctx.lineWidth=2+chargeProgress*2;ctx.setLineDash([5,8]);ctx.beginPath();ctx.moveTo(s.source?.x??s.x,s.source?.y+18??s.y);ctx.lineTo(s.x,s.y);ctx.stroke();ctx.setLineDash([]);for(let ring=0;ring<3;ring++){ctx.globalAlpha=(.25+chargeProgress*.45)*(1-ring*.2);ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(s.x,s.y,(s.r+9+ring*9)*pulse*(1-chargeProgress*.18),0,Math.PI*2);ctx.stroke();}ctx.restore();});
