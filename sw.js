...
const keys = new Set();
const pointer = { active:false, id:null, baseX:0, baseY:0, knobX:0, knobY:0, dx:0, dy:0, radius:56 };
...
function reset() {
  player = { characterId:'default', x:W/2, y:H*.78, r:18, speed:460, hp:100, maxHp:100, fire:.31, shot:0, damage:18, multishot:1, level:1, xp:0, need:35, inv:0, diving:false, altitude:1, diveTime:0, diveCooldown:0, rerolls:2, ultimateGauge:0, ultimateMax:characterTypes.default.ultimate.maxGauge,
  ...
  diveButton.classList.remove('visible');
...
function updateUltimateButton(){
  ...
  ultimateButton.disabled=state!=='play'||ratio<1||player.diving;
...
function useUltimate(){
  if(state!=='play'||player.diving||player.ultimateGauge<player.ultimateMax)return;
...
function updatePlayerMovement(dt){
  ...
  player.x=clamp(player.x,38,W-38);player.y=clamp(player.y,105,H-300);
}
...
function update(dt){
  ...
  bgY=(bgY+52*dt*(player.diving?.7:1))%bgTileHeight; player.inv-=dt;player.hitFlash=Math.max(0,(player.hitFlash||0)-dt);
  if(player.diving){
  ...
  if(!player.diving)player.wingSlots.forEach((wing,index)=>{if(wing.shot>0)return;if(wing.type==='ignis')fireIgnis(index,wing);else if(wing.type==='voltis')fireVoltis(index,wing);else if(wing.type==='venora')fireVenora(index,wing);});
  ...
  if(!player.diving&&player.altitude>.82)for(const e of enemies)if(!e.dead&&dist2(player,e)<(player.r+e.r)**2){e.dead=true;if(player.inv<=0&&damagePlayer(e.damage,.75,'#fff'))return;}
...
function endGame(){releaseVirtualJoystick();state='over';ui.result.textContent=t('result.summary',{seconds:Math.floor(time),score,wave});ui.over.classList.add('visible');diveButton.classList.remove('visible');ultimateButton.classList.remove('visible');hud.root.classList.remove('visible');}
...
function updateDiveButton(){
  ...
  if(player.diving){
    ...
    icon.textContent='▼';label.textContent=t('dive.active',{seconds:player.diveTime.toFixed(1)});
    ...
    icon.textContent='◷';label.textContent=t('dive.cooldown',{seconds:player.diveCooldown.toFixed(1)});
    diveButton.style.background=`conic-gradient(#7d9aa6 ${ratio*360}deg,#354a53 0deg)`;
    diveButton.setAttribute('aria-disabled','true');
  }else{
    icon.textContent='▼';label.textContent=t('dive.ready');
...
document.querySelector('#backLobby').onclick=()=>{releaseVirtualJoystick();state='lobby';ui.over.classList.remove('visible');ui.lobby.classList.add('visible');hud.root.classList.remove('visible');diveButton.classList.remove('visible');ultimateButton.classList.remove('visible');};
document.querySelector('#pause').onclick=()=>{if(state==='play'){releaseVirtualJoystick();state='paused';ui.pause.classList.add('visible');diveButton.classList.remove('visible');ultimateButton.classList.remove('visible');}};
document.querySelector('#resume').onclick=()=>{if(state==='paused'){state='play';ui.pause.classList.remove('visible');diveButton.classList.add('visible');ultimateButton.classList.add('visible');last=performance.now();}};
...
canvas.addEventListener('pointerdown',e=>{if(state!=='play'||pointer.active)return;e.preventDefault();const p=canvasPoint(e);const bx=clamp(p.x,88,W-88),by=clamp(p.y,H-260,H-88);pointer.active=true;pointer.id=e.pointerId;pointer.baseX=pointer.knobX=bx;pointer.baseY=pointer.knobY=by;pointer.dx=pointer.dy=0;canvas.setPointerCapture?.(e.pointerId);});
